...
export default function AdminOrders() {
  ...
  const load = async () => {
    ...
    setDrivers(dRes.data.data.filter(d => d.isActive))
    setLoading(false)
  }
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader title="All Orders" subtitle={`${orders.length} orders in the system`} />

      <div className="flex gap-1.5 flex-wrap mb-5">
        {STATUSES.map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border
              ${filter === s ? 'bg-primary text-white border-primary' : 'bg-white/3 text-slate-500 border-white/6 hover:border-white/15'}`}>
            {s}
          </button>
        ))}
      </div>
      ...
      {loading
        ...
        : orders.length === 0
          ? <Empty icon={Package} message="No orders match this filter" />
          : (
              ...
              {orders.map(o => (
                <div key={o._id} className="card p-4 flex items-center gap-3 flex-wrap">
                  <div className="flex-1 min-w-[160px]">
                    ...
                    <div className="text-[11px] text-slate-500 mt-0.5">{o.shop?.district} · {o.createdAt?.slice(0, 10)}</div>
                  </div>
                  <div className="text-center">
                    <div className="font-mono text-xl font-bold text-primary">{o.quantity}</div>
                    <div className="text-[10px] text-slate-600">barrels</div>
                  </div>
                  {o.assignedDriver ? (
                    <div className="text-center">
                      <div className="font-mono text-xs font-bold text-orange-400">{o.plateNumber}</div>
                  ...
                  <div className="flex gap-2 ml-auto">
                    ...
                    <Button size="sm" variant="danger" onClick={() => handleDelete(o._id)}>
                      <Trash2 size={12} />
                    </Button>
                  </div>
                </div>
